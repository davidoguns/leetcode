...
public:
    int maxArea(vector<int>& height) {
        ...
        //place initial check at front
        s.push(make_tuple(0, height.size()-1));
        int max_area = 0;

        while (!s.empty()) {
            auto entry = s.top();
            int front_index = get<0>(entry);
            int back_index = get<1>(entry);
            const tuple<int, int> area_move_key = make_tuple(front_index, back_index);

            if (memo.find(area_move_key) != memo.end()) {
                s.pop();
                continue;
            }
           
            if (back_index - front_index == 1) {
                //no next moves to calculate, this is terminal
                int area_local = (back_index - front_index) * std::min(height[front_index], height[back_index]);
                max_area = std::max(max_area, area_local);
                memo.insert(make_pair(area_move_key, area_local));
                s.pop();
            }
            else {
                tuple<int, int> front_move_key = make_tuple(front_index+1, back_index);
                tuple<int, int> back_move_key = make_tuple(front_index, back_index-1);
                const auto back_move_max_area = memo.find(back_move_key);
                const auto front_move_max_area = memo.find(front_move_key);
                if (back_move_max_area == memo.end()) {
                    s.push(back_move_key);
                }
                if (front_move_max_area == memo.end()) {
                    s.push(front_move_key);
                }

                if (front_move_max_area != memo.end() && back_move_max_area != memo.end()) {
                    int area_local = (back_index - front_index) * std::min(height[front_index], height[back_index]);
                    int max_area_local = std::max(area_local, std::max(front_move_max_area->second, back_move_max_area->second));
                    max_area = std::max(max_area, max_area_local);
                    memo.insert(make_pair(area_move_key, max_area_local));
                    s.pop();
                } else { }
            }
        }
