...
private:
    void bottom_up_count(multimap<int, int> const& map, int key, int level, int& count) {
        if (level == 1) {
            ...
            if (map_itr != map.upper_bound(key)) { //check if key is found in map
                //otherwise, keep scanning up through all nodes that mapped to previous
                //and subtract one from level since we traversed a hop
